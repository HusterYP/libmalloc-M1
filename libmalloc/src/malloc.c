...
{
#if CONFIG_FEATUREFLAGS_SIMPLE
	bool zero_on_free_feature_enabled = true; //os_feature_enabled_simple(libmalloc, ZeroOnFree, MALLOC_ZERO_POLICY_DEFAULT == MALLOC_ZERO_ON_FREE);
...
#if TARGET_OS_OSX
	bool allow_non_platform = true; // os_feature_enabled_simple(libmalloc, SecureAllocator_NonPlatform, false);
...
__malloc_init(const char *apple[])
{
	...
	if (getpid() != 1) {
		allow_bootargs &= true; // os_feature_enabled_simple(libmalloc, EnableBootArgs, false);
	}
#endif
...
// cooci 和谐学习 不急不躁 这里增设_malloc_initialize_once

static void
_malloc_initialize_wrapper(void *p)
{
	_malloc_initialize("NULL", "NULL");
}
...
{
	os_once(&_malloc_initialize_pred, NULL, _malloc_initialize_wrapper);
}
